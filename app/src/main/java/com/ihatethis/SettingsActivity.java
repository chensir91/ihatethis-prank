...
public class SettingsActivity extends Activity {
    ...
    private void buildUI() {
        ...
        flPreview = new FrameLayout(this);
        flPreview.setBackgroundColor(Color.parseColor("#1A000000"));
        flPreview.setLayoutParams(previewLP);
        ...
        previewText.setLayoutParams(textLP);
        flPreview.addView(previewText);
        root.addView(flPreview);

        sbSimPlay = makeSB(0, 100);
        btnSimPlay = btn("模拟播放");
        root.addView(sbSimPlay); root.addView(btnSimPlay);
        ...
        sbSimPlay.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            public void onProgressChanged(SeekBar s, int v, boolean u) { updatePreviewScreen(); }
            public void onStartTrackingTouch(SeekBar s) {}
            public void onStopTrackingTouch(SeekBar s) {}
        });
        ...
        btnSimPlay.setOnClickListener(v -> simAnimate());
    ...
    private void updatePreviewScreen() {
        float p = sbSimPlay.getProgress() / 100f;
        int minF = sm.getInterpolatedMinFont(p);
        int maxF = sm.getInterpolatedMaxFont(p);
        int fs = minF + (maxF - minF) / 2;
        int c = sm.getFontColor();
        ...
        for (String t : texts) if (t != null && !t.isEmpty()) nonEmpty.add(t);
        if (nonEmpty.isEmpty()) {
            previewText.setText("(空)");
            previewText.setRotation(0);
            return;
        }
        String txt = nonEmpty.get(new java.util.Random().nextInt(nonEmpty.size()));
        previewText.setText(txt);
        previewText.setTextSize(fs);
        previewText.setTextColor(c);
        previewText.setRotation(dir);
    }
    ...
    private void simAnimate() {
        ...
        currentSimText = nonEmpty.get(new java.util.Random().nextInt(nonEmpty.size()));
        int speed = sm.getTypingSpeedMs();
        int duration = sm.getDisplayDurationMs();
        int fs = sm.getMinFontStart() + (sm.getMaxFontStart() - sm.getMinFontStart()) / 2;
        int c = sm.getFontColor();
